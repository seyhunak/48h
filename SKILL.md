...
- **Data:** **`/data` page (Clerk-protected, add to `createRouteMatcher`)** — full CRUD over the Convex tables (tenants, users, invoices, submissions, credits, vault + your [DOMAIN] tables) — tenant-scoped queries (users see own tenant rows; `role=admin` sees all), realtime Convex queries, server-side validation, delete requires confirm + writes an audit entry. No dummy rows; empty state shows "no records yet" + CTA into the `/app` workflow.
