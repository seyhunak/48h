...
name: 48h
description: >-
  48-hour sprint to land a first paying customer (aka 48h Money In). Always
  starts by asking domain(area), location/region, pricing target, and platform
  (Web SaaS vs mobile). Supports Web (Next.js+Convex+Stripe) and mobile ideas
  (Flutter+Firebase+RevenueCat IAP, iOS+Android). Autonomously builds and
  deploys the app (production deploy requires human approval) and operates it
  like real business ops — maintenance, optimization, marketing, sales, CRM —
  for new or existing 48h apps. Build discipline is addyosmani/agent-skills
  (spec-driven-development, plan, build, test, review, ship; spec-kit retired).
  Use when the user invokes a 48h sprint with domain, location and price range,
  or provides a URL to capture domain/target/pricing opportunities, or asks to
  find/validate/sell a solution to a real expensive problem and close a paying
  customer within 48 hours, or asks to
  build/deploy/operate/maintain/market/sell an existing 48h app. Do not use
  for long product development or general brainstorming.
---
...
### Track A — Web SaaS (default) — Spec-Driven via agent-skills + Hallmark UI

When you do build, **always kick off Track A with spec-driven development: install agent-skills + Hallmark first, then spec → Hallmark UI → code.** After building, run §15 SHIP TO GITHUB to seed, set up .envs, and push to a GitHub repo (created if needed).

- **0. Kickoff installs (mandatory, first commands in fresh project root):**
  ```bash
  # agent-skills — production-grade engineering skills (https://github.com/addyosmani/agent-skills)
  # Replaces spec-kit (retired — no specify-cli, no .specify/, no /speckit-*).
  ...
  ```
  Verify: `npx skills list` shows both packs + Hallmark present (`ls .agents/skills/spec-driven-development .agents/skills/hallmark 2>/dev/null || ls ~/.claude/skills/spec-driven-development ~/.claude/skills/hallmark/ 2>/dev/null`) — re-run installers if missing. Never write app code before the spec exists and Hallmark is installed. Code only via opencode/kilo/cline TUIs (§5 builder stack) with `model:approve`.
- **Spec first (agent-skills SDD):** route via the `using-agent-skills` skill, then run the lifecycle one phase at a time — `spec-driven-development` (feed it the §3 SELECT ONE output: problem → buyer → scope → success criteria; save `SPEC.md`, get approval) → `planning-and-task-breakdown` (save `tasks/plan.md` + `tasks/todo.md`, get approval) → `incremental-implementation` + `test-driven-development` (one slice at a time, red → green → commit per task) → `debugging-and-error-recovery` on failures → `code-review-and-quality` (+ `security-and-hardening` / `performance-optimization` on those axes) → `shipping-and-launch` (GO/NO-GO + rollback). Spec is the source of truth; Hallmark builds UI from it. Idea scoring/kill discipline: `idea-refine` + `interview-me` (use at §2–§3 to kill weak ideas with evidence, ending in go / clarify / kill). Bug lane: `debugging-and-error-recovery` with the Prove-It pattern (failing repro test → fix → green; use in §16 Maintain).
...
When you do build, **suggest generating the app spec-first** (same SDD discipline as Track A: `spec-driven-development` with problem → buyer → scope → success criteria → paywall), then build with Flutter (single codebase, both stores):

- **Spec first:** `spec-driven-development` writes the 48h mobile fix-pack spec (problem → user → single core loop → paywall → success metric) before code — matches §3 SELECT ONE. Scope to ONE core loop + paywall, nothing else.
